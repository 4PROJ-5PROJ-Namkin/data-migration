...
![alt text](https://i.imgur.com/aikFzaR.jpg)
### Global Pipeline
...
After ingesting data in the ODS tables, for each table, you'll have to duplicate it in the DWH and to prepare the calculation and the aggregation of some useful measures for the fact tables that'll be utilized later in the dashboards.

Run this command:
```
sqlcmd -S <your-local-machine> -E -i "./scripts/infrastructure/dwh/dwh_create_and_bulk_massive_inserts_<table-name>_table.sql
```
...
### Reporting
Connect the DWH with Power BI Desktop and load the ``PBIX`` file located in the ``dashboards`` folder.
